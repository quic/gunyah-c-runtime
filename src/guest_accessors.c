...
{
	return ((b1.bf[0] & 0x8000000000000f3fU) ==
		(b2.bf[0] & 0x8000000000000f3fU));
}

bool
vcpu_option_flags_is_empty(vcpu_option_flags_t bit_field)
{
	return ((bit_field.bf[0] & 0x8000000000000f3fU) == 0U);
}

bool
vcpu_option_flags_is_clean(vcpu_option_flags_t bit_field)
{
	return ((bit_field.bf[0] & 0x7ffffffffffff0c0U) == 0x0U);
}
...
error_raw_cast(int32_t val)
{
	...
	case (int32_t)ERROR_MSGQUEUE_FULL:
		...
		break;
	case (int32_t)ERROR_LPI_NOT_ASSIGNED:
		ret = ERROR_LPI_NOT_ASSIGNED;
		break;
	case (int32_t)ERROR_NO_PENDING_VLPI:
		ret = ERROR_NO_PENDING_VLPI;
		break;
...
error_raw_cast_safe(int32_t val)
{
	...
	case (int32_t)ERROR_MSGQUEUE_FULL:
		...
		break;
	case (int32_t)ERROR_LPI_NOT_ASSIGNED:
		ret = error_result_ok(ERROR_LPI_NOT_ASSIGNED);
		break;
	case (int32_t)ERROR_NO_PENDING_VLPI:
		ret = error_result_ok(ERROR_NO_PENDING_VLPI);
		break;
	case (int32_t)ERROR_CSPACE_CAP_NULL:
